...
4. Since we do not have gcc on the target, we have to compile on our kali
	...
	4.  Transfer `/tmp/ofs-lib.c` to kali
	...
	5.  Compile `/tmp/ofs-lib.c` on kali

		```
		gcc -m64 -fPIC -shared -o ofs-lib.so ofs-lib.c -ldl -w
		```

	6. Remove `Line 138-147` from `37292.c`
	...
	8. Transfer `exploitNew` & `ofs-lib.so` to target
	...
	11. After reading writeups, you can just change `gcc` to `cc` in `37292.c`
