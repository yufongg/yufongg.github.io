...
3. Enumerate wordpress
	...
	1. Enumerate plugins (aggressively)
...
4. Try [POC of WP Core 5.6.2 Authenticated XXE (CVE-2021-29447)](https://blog.wpsec.com/wordpress-xxe-in-media-library-cve-2021-29447/)
	...
	1. To proceed, we have to include something useful, the next thing that comes to my mind is to include `wp-config.php`, as it contains credentials. However, we do not know the full path of the web root directory. Let's find out!
	2. Earlier, we found out that the web server is running 

		> - `nginx 1.18.0` 
		> - Virtual Host is used (due to redirecting from metatwo.htb -> metapress.htb)
		{: .prompt-info}

	1. We are able to find out the root web directory by including the virtual host configuration file 
...
3. SQLMap
	...
	1. Enumerate tables in `blog` database
