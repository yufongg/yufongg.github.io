...
img_path: /_posts/Writeups/HackTheBox/Linux/Node/images/
image:
  path: /_posts/Writeups/HackTheBox/Linux/Node/images/Pasted%20image%2020220828004917.png
...
1. Proceed to `http://node.htb/login`, determine if page is susceptible to SQLi Auth Bypass
	![]({{ page.img_path }}Pasted%20image%2020220313180212.png)
	- Failed
2. Intercept w/ burpsuite
	![]({{ page.img_path }}Pasted%20image%2020220313180354.png)
...
4. View files that are queried on this website
	- `Inspect Element -> Sources -> js -> app -> controllers`
		![]({{ page.img_path }}Pasted%20image%2020220313181248.png)
...
6. View enumerated directories	
	- `session`
		![]({{ page.img_path }}Pasted%20image%2020220313181914.png)
	- `users`
		``` 
		┌──(root💀kali)-[~/htb/node]
		...
		```
		![]({{ page.img_path }}Pasted%20image%2020220313181407.png)
...
1. Login w/ `myP14ceAdm1nAcc0uNT:manchester` & download `Backup`
	![]({{ page.img_path }}Pasted%20image%2020220313183306.png)
...
4. After browsing through the files, SSH credentials in `app.js`
	![]({{ page.img_path }}Pasted%20image%2020220313184940.png)
...
1. SSH w/ `mark:5AYRft73VtFpc84k`
	...
	```
	![]({{ page.img_path }}Pasted%20image%2020220827175638.png)
...
2. Found something interesting w/ `linpeas`
	![]({{ page.img_path }}Pasted%20image%2020220313193217.png)
...
5. Exploiting `scheduler`
	...
	3. Add value into `cmd` from `task` collection that will execute our reverse shell payload we just created
		...
		```
	4. Shell obtained
		![]({{ page.img_path }}Pasted%20image%2020220827175108.png)
6. User Flag
	``` 
...
3. Earlier, `app.js` revealed Mark's credentials, it also contains information about the binary `backup`
	![]({{ page.img_path }}Pasted%20image%2020220313220503.png)
...
7. Command Injection
	...
	2. Do command injection w/ `printf`
		...
		```
		![]({{ page.img_path }}Pasted%20image%2020220827200528.png)
	3. Do command injection w/ `'`
		...
		```
		![]({{ page.img_path }}Pasted%20image%2020220827200723.png)
8. Root Flag
	```
	root@node:/home/mark# cat /root/root.txt
...
2. Use wildcards `*`,`?` to backup `/root`
	...
	```
	![]({{ page.img_path }}Pasted%20image%2020220827211453.png)
3. Decode & Unzip
	```
	┌──(root💀kali)-[~/htb/node/10.10.10.58/loot]
...
2. Change our HOME ENV and backup 
	...
	```
	![]({{ page.img_path }}Pasted%20image%2020220827211313.png)
3. Decode and Unzip
...
1. `/root` is not filtered, proceed to `/` and backup `root`
	...
	```
	![]({{ page.img_path }}Pasted%20image%2020220827211546.png)
2. Decode and Unzip
...
3. Compile & Exploit
	...
	```
	![]({{ page.img_path }}Pasted%20image%2020220827212920.png)
...
5. Use [binary ninja](https://cloud.binary.ninja/) to examine `backup` to determine the vulnerable function
	1. After selecting the binary, click on `strcpy -> displayTarget`
		![]({{ page.img_path }}Pasted%20image%2020220827224510.png)
	2. `Double-Click` to view `displayTarget`
		![]({{ page.img_path }}Pasted%20image%2020220827223307.png)
		```
		sub esp, 0x208 - Make space on ESP (Effective Stack Pointer), 208 (HEX) -> 520 Bytes
		```
		- Able to bufferoverflow at 500-600bytes.
		- We have to figure out how to trigger the displayTarget output `Starting archiving ...`
	3. Figure out how trigger Starting archiving... by playing w/ the options
		```
		# Does not trigger it
...
6. Buffer Overflow
	...
	2. Determine Pattern Address
		...
		```
		![]({{ page.img_path }}Pasted%20image%2020220827233050.png)
