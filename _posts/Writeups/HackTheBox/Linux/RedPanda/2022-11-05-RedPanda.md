...
1. Lets analyze `file.jar` w/ `jd-gui`
	...
	> 2. When executed, reads `redpanda.log` line by line.
	> 3. Checks if line contains string `.jpg` 
	> 4. Splits the line into  parts w/ `||` as a delimiter
	> - `[0]: Status Code`
	> - `[1]: Ip Address`
	> - `[2]: User Agent`
	> - `[3]: URI (Name of image file)`
    > 5. Get artist name using metadata (`Artist:`) of image file
    > 6. Create `xml` path
    > - `/credits/ + artist + _creds.xml` 	
    > 7. Process `xml` file and update it by writing it.
	{: .prompt-info}	
