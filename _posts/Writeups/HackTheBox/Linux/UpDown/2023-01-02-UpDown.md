...
4. Found another interesting commit `f67efd00c10784ae75bd251add3d52af50d7addd - Create checker.php`
	...
	> 5. Basically what the `PHP` code is doing is, 
	> 6. Checks if the file we uploaded is less than `10 kb`, else `die`
	> 7. Checks if the content of the file we uploaded, whether it has any banned extensions (This is to prevent `PHP` file from being uploaded). However, `.phar` is not in the list, this means that we can upload a `.phar` file to do code execution.
	> 8. Next, a directory (`uploads/md5sum/<uploaded file>`) is created to store our uploaded file.
	> 9. Next, it `curl` the URLs in our file.
	> 10. After the `for` loop goes through all the URLs, delete the uploaded file.
	{: .prompt-info }

11. How do we do a file upload bypass to execute `PHP` code
